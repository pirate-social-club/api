...
Most TypeScript service checks use the TypeScript 7 native compiler (`tsc`). Prefer service-local `rtk bun run check` for focused work on weak machines; the repo-level `rtk bun run check` still chains hygiene, shared, contracts, agent connector, issuer, ZKPassport container, and API checks.
