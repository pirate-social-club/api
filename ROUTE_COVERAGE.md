...
| `/operator/content-security/scanner-releases*` | `src/routes/content-security-ops.ts` | `src/routes/content-security-ops.test.ts` | direct/service | Dedicated operator scope, strict release identity parsing, audited staging, atomic activation/retirement, and terminal revocation. |
...
| `/communities/*` commerce paths | `src/routes/communities-commerce.ts` | `tests/routes/song-artifacts/song-artifact-routes.test.ts`, `tests/routes/song-artifacts/song-artifact-locked-routes.test.ts`, `tests/routes/song-artifacts/song-artifact-donation-routes.test.ts`, `tests/community-commerce-*.test.ts`, `tests/community-commerce-generic-digital-goods.test.ts` | direct/partial | Includes the security-critical generic flow (blob → listing → purchase → entitlement → access/content) and the no-entitlement access denial; Story/CDR route paths are mapped in `STORY_CDR_PATHS.md`. |
...
| `/communities/*/content-blobs*` | `src/routes/communities-content-blobs.ts` | `tests/routes/content-blobs/routes.test.ts`, `src/lib/content-blobs/content-blob-service.test.ts` | direct | Server-flagged transport foundation; owner create/upload/read lifecycle plus policy and fail-closed safety state. No asset publication is enabled. |
