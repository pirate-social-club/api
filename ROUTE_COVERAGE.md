...
| `/posts/:postId/dance-choreography` | `src/routes/post-dance-choreographies.ts` | `src/lib/dance/choreography-reference-repository.test.ts`, `src/lib/dance/choreography-reference-storage.test.ts` | service/partial | Authenticated post alias for active, ready choreography references. Repository selection and short-lived playback signing have direct service coverage; route-level auth and serialization coverage remains before pilot enablement. |
