/**
 * Build-time generator for the D1-native schema-load path (workstream §8.7,
 * option 2 — final-schema-dump).
 *
 * Replays the core/db/community-template migrations into a throwaway in-memory
 * libsql DB, then dumps the FINAL-form schema as CREATE TABLE / INDEX / TRIGGER
 * statements (no ALTER/DROP/PRAGMA — those run only here, in this trusted build
 * step). Every generated statement is asserted against the shard bootstrap
 * guard (isBootstrapAllowedStatement) below — generation FAILS if any
 * statement would be rejected — so the d1_native path can load a full
 * community schema with no snapshot/guard drift. (Migration 1147's CREATE
 * TRIGGERs, whose BEGIN ... END body semicolons the guard used to reject,
 * took down production provisioning while the freshness check stayed green;
 * the assertion is what makes that class of drift a build-time failure.)
 *
 * Also emits the migration name+checksum list so the d1 path can seed
 * `schema_migrations` (the operator records these; schema-state checks rely on it).
 *
 * Run: `bun run scripts/generate-community-schema-snapshot.ts`
 * (re-run whenever the community-template migrations change).
 */
import { createClient } from "@libsql/client"
// Use the source module directly: this script runs in CI before API workspace
// dependencies are installed, while the shared source is already checked out.
import { isBootstrapAllowedStatement } from "../../shared/src/sql-read-guard"
import {
  BOOTSTRAP_STATE_TABLE_DDL,
  SCHEMA_ATTESTATION_INVENTORY_SQL,
  SCHEMA_ATTESTATION_MIGRATION_LEDGER_SQL,
  shardSchemaObservationProof,
  type ShardMigrationLedgerRow,
  type ShardSchemaInventoryRow,
} from "../../shared/src/schema-attestation"
import { createHash } from "node:crypto"
import { mkdir, readdir, readFile, writeFile } from "node:fs/promises"
import { dirname, join } from "node:path"
import { fileURLToPath } from "node:url"
import { resolveCoreRepoPath } from "../shared/core-repo-paths"
import { splitSqlStatements, toSqliteCompatibleStatements } from "../shared/sql-migration"

const SCHEMA_MIGRATIONS_DDL = `CREATE TABLE IF NOT EXISTS schema_migrations (
  migration_name TEXT PRIMARY KEY,
  migration_label TEXT NOT NULL,
  checksum TEXT NOT NULL,
  applied_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
)`

const OUT_REL = "../src/lib/communities/provisioning/generated/community-schema-snapshot.ts"

async function main(): Promise<void> {
  const migrationsDir = resolveCoreRepoPath("db/community-template/migrations")
  const files = (await readdir(migrationsDir)).filter((f) => f.endsWith(".sql")).sort()

  const db = createClient({ url: ":memory:" })
  await db.execute(SCHEMA_MIGRATIONS_DDL)

  const migrations: { name: string; checksum: string }[] = []
  for (const file of files) {
    const sql = await readFile(join(migrationsDir, file), "utf8")
    migrations.push({ name: file, checksum: createHash("sha256").update(sql).digest("hex") })
    for (const raw of splitSqlStatements(sql)) {
      for (const stmt of toSqliteCompatibleStatements(raw)) {
        const trimmed = stmt.trim()
        if (!trimmed) continue
        const verb = trimmed.slice(0, 6).toUpperCase()
        if (verb === "BEGIN " || trimmed.toUpperCase() === "COMMIT" || trimmed.toUpperCase() === "BEGIN") continue
        await db.execute(trimmed)
      }
    }
  }

  const rows = (
    await db.execute(
      "SELECT type, name, sql FROM sqlite_master WHERE sql IS NOT NULL " +
        "AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '_cf_%' " +
        "ORDER BY CASE type WHEN 'table' THEN 0 ELSE 1 END, name",
    )
  ).rows
  const statements = rows.map((r) => `${String(r.sql).trim()};`)

  // Expected value only: the shard still has to query the real target after
  // loading it. Include the same bootstrap marker and migration ledger that
  // exist in a freshly provisioned community D1.
  await db.execute(BOOTSTRAP_STATE_TABLE_DDL)
  for (const migration of migrations) {
    await db.execute({
      sql: "INSERT INTO schema_migrations (migration_name, migration_label, checksum) VALUES (?1, 'community-template', ?2)",
      args: [migration.name, migration.checksum],
    })
  }
  const attestationSchemaRows = (await db.execute(SCHEMA_ATTESTATION_INVENTORY_SQL)).rows.map((row) => ({
    type: String(row.type) as "index" | "table",
    name: String(row.name),
    sql: row.sql == null ? null : String(row.sql),
  })) satisfies ShardSchemaInventoryRow[]
  const attestationMigrationRows = (await db.execute(SCHEMA_ATTESTATION_MIGRATION_LEDGER_SQL)).rows.map((row) => ({
    migration_name: String(row.migration_name),
    checksum: String(row.checksum),
  })) satisfies ShardMigrationLedgerRow[]
  const observationProof = await shardSchemaObservationProof({
    schemaRows: attestationSchemaRows,
    migrationLedgerRows: attestationMigrationRows,
  })

  // Hard contract, enforced — not a comment. Every emitted statement must pass
  // the shard bootstrap guard, or communityD1LoadSnapshot will reject the load
  // in production. Fail generation (and --check, which CI runs) loudly instead.
  const guardRejected = statements.filter((s) => !isBootstrapAllowedStatement(s))
  if (guardRejected.length > 0) {
    console.error(
      `community schema snapshot contains ${guardRejected.length} statement(s) ` +
        "the shard bootstrap guard (isBootstrapAllowedStatement) would reject:",
    )
    for (const s of guardRejected) console.error(`---\n${s}`)
    process.exit(1)
  }

  const body =
    `// GENERATED by scripts/generate-community-schema-snapshot.ts — DO NOT EDIT.\n` +
    `// Final-form community-local schema (CREATE-only), produced by replaying the\n` +
    `// ${files.length} core/db/community-template migrations into an in-memory DB and\n` +
    `// dumping sqlite_master. Used by the d1_native provisioning path (§8.7) to load\n` +
    `// a full community schema via communityD1LoadSnapshot. Every statement below is\n` +
    `// asserted against the shard bootstrap guard (isBootstrapAllowedStatement) at\n` +
    `// generation time — no ALTER/DROP/PRAGMA reaches the shard, only these CREATEs.\n\n` +
    `export const COMMUNITY_SCHEMA_STATEMENTS: readonly string[] = ${JSON.stringify(statements, null, 2)}\n\n` +
    `export const COMMUNITY_SCHEMA_MIGRATIONS: readonly { name: string; checksum: string }[] = ${JSON.stringify(migrations, null, 2)}\n\n` +
    `export const COMMUNITY_SCHEMA_OBSERVATION_PROOF = ${JSON.stringify(observationProof, null, 2)} as const\n`

  const outPath = join(dirname(fileURLToPath(import.meta.url)), OUT_REL)

  // Explicit ledger-coverage assertion: every core template migration must be recorded.
  const covered = new Set(migrations.map((m) => m.name))
  const uncovered = files.filter((f) => !covered.has(f))
  if (uncovered.length > 0) {
    console.error(`community schema snapshot ledger is missing migrations: ${uncovered.join(", ")}`)
    process.exit(1)
  }

  if (process.argv.includes("--check")) {
    let existing = ""
    try {
      existing = await readFile(outPath, "utf8")
    } catch {
      existing = ""
    }
    if (existing !== body) {
      console.error("community schema snapshot is STALE (does not match a fresh regeneration from core).")
      console.error("Regenerate and commit it:")
      console.error("  PIRATE_CORE_REPO=<core checkout> bun run scripts/generate-community-schema-snapshot.ts")
      console.error(`Expected ${statements.length} schema statements and ${migrations.length} migrations from ${files.length} core template files.`)
      process.exit(1)
    }
    console.log(`community schema snapshot is fresh: ${migrations.length} migrations, ${statements.length} schema statements`)
    return
  }

  await mkdir(dirname(outPath), { recursive: true })
  await writeFile(outPath, body)
  console.log(`generated: ${statements.length} schema statements, ${migrations.length} migrations from ${files.length} files`)
}

await main()
