...
import { renewHnsOwnershipLease } from "../src/lib/hns-root-observer/ownership-lease-renewal"
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    "Usage: bun run scripts/admin-hns-ownership-lease-renew.ts --root ROOT --actor ACTOR --reason REASON --verifier-base-url URL [--apply]",
  )
  process.exit(0)
}
...
const result = await withStandaloneControlPlaneClient(env, async (client) => {
  return await renewHnsOwnershipLease({
