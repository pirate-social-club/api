...
} from "../lib/communities/board-read/board-read-service"
...
function mcpBoardReadPost(c: McpContext, post: BoardReadPost | BoardReadPostSearchResult) {
  const id = publicPostId(post.postId)
  return {
    id,
...
function mcpBoardReadComment(c: McpContext, comment: BoardReadComment) {
...
async function callSearchBoardTool(c: McpContext, rawArgs: unknown) {
...
async function callGetThreadTool(c: McpContext, rawArgs: unknown) {
...
async function callGetMyActivityTool(c: McpContext, rawArgs: unknown) {
