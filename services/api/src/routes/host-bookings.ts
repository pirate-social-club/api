...
import * as hostAuthoring from "../lib/bookings/host-authoring-service"
import {
...
hostBookings.get("/me/profile", async (c) => {
  ...
  const profile = await hostAuthoring.getBookingProfile(c.env, actor.userId)
  if (!profile) {
...
hostBookings.post("/me/profile", async (c) => {
  ...
  const result = await hostAuthoring.upsertBookingProfile(
    c.env,
    actor.userId,
    input as Parameters<typeof hostAuthoring.upsertBookingProfile>[2],
  )
  if (!result.ok) {
...
hostBookings.post("/me/profile/publish", async (c) => {
  const actor = c.get("actor")
  const result = await hostAuthoring.setProfilePublished(c.env, actor.userId, true)
  if (!result.ok) {
...
hostBookings.post("/me/profile/unpublish", async (c) => {
  const actor = c.get("actor")
  const result = await hostAuthoring.setProfilePublished(c.env, actor.userId, false)
  if (!result.ok) {
    return c.json({ error: result.reason }, errStatus(result.reason))
  }
  return c.json(serializeBookingProfile(result.data), 200)
...
hostBookings.get("/me/availability-rules", async (c) => {
  const actor = c.get("actor")
  const rules = await hostAuthoring.listAvailabilityRules(c.env, actor.userId)
...
hostBookings.post("/me/availability-rules", async (c) => {
  ...
  const result = await hostAuthoring.createAvailabilityRule(c.env, actor.userId, input as never)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.post("/me/availability-rules/:ruleId", async (c) => {
  ...
  const result = await hostAuthoring.updateAvailabilityRule(c.env, actor.userId, ruleId, input as never)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.delete("/me/availability-rules/:ruleId", async (c) => {
  const actor = c.get("actor")
  const ruleId = c.req.param("ruleId")
  const deleted = await hostAuthoring.deleteAvailabilityRule(c.env, actor.userId, ruleId)
  if (!deleted) {
...
hostBookings.get("/me/availability-exceptions", async (c) => {
  const actor = c.get("actor")
  const exceptions = await hostAuthoring.listAvailabilityExceptions(c.env, actor.userId)
...
hostBookings.post("/me/availability-exceptions", async (c) => {
  ...
  const result = await hostAuthoring.createAvailabilityException(c.env, actor.userId, input as never)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.post("/me/availability-exceptions/:exceptionId", async (c) => {
  ...
  const result = await hostAuthoring.updateAvailabilityException(c.env, actor.userId, exceptionId, input as never)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.delete("/me/availability-exceptions/:exceptionId", async (c) => {
  const actor = c.get("actor")
  const exceptionId = c.req.param("exceptionId")
  const deleted = await hostAuthoring.deleteAvailabilityException(c.env, actor.userId, exceptionId)
  if (!deleted) {
    return c.json({ error: "not_found" }, 404)
  }
...
hostBookings.get("/me/price-rules", async (c) => {
  const actor = c.get("actor")
  const rules = await hostAuthoring.listPriceRules(c.env, actor.userId)
...
hostBookings.post("/me/price-rules", async (c) => {
  ...
  const result = await hostAuthoring.createPriceRule(c.env, actor.userId, input as never, priority)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.post("/me/price-rules/:priceRuleId", async (c) => {
  ...
  const result = await hostAuthoring.updatePriceRule(c.env, actor.userId, priceRuleId, input as never)
  if (!result.ok) {
    return c.json({ error: result.reason, ...(result.fields ? { fields: result.fields } : {}) }, errStatus(result.reason))
  }
...
hostBookings.delete("/me/price-rules/:priceRuleId", async (c) => {
  const actor = c.get("actor")
  const priceRuleId = c.req.param("priceRuleId")
  const deleted = await hostAuthoring.deletePriceRule(c.env, actor.userId, priceRuleId)
  if (!deleted) {
    return c.json({ error: "not_found" }, 404)
  }
