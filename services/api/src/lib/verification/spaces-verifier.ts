...
type SpacesVerifyPublishResponse = {
  fabric_publish_verified?: boolean
  root_key_proof_verified?: boolean | null
  web_target_verified?: boolean | null
  freedom_target_verified?: boolean | null
  observed_web_url?: string | null
  observed_freedom_url?: string | null
  observed_txt_values?: string[] | null
...
export type SpacesPublishVerification = {
  fabricPublishVerified: boolean
  rootKeyProofVerified: boolean
  webTargetVerified: boolean
  freedomTargetVerified: boolean
  observedWebUrl: string | null
  observedFreedomUrl: string | null
...
export async function verifySpacesFabricPublish(
  env: Env,
  input: {
    rootLabel: string
    txtKey: string
  ...
  const result = await spacesVerifierRequest<SpacesVerifyPublishResponse>(env, {
    ...
    body: {
      ...
      txt_key: input.txtKey,
      txt_value: input.txtValue,
      web_url: input.webUrl,
      freedom_url: input.freedomUrl,
    },
  })
  ...
  return {
    fabricPublishVerified: result.fabric_publish_verified === true,
    rootKeyProofVerified: result.root_key_proof_verified === true,
    webTargetVerified: result.web_target_verified === true,
    freedomTargetVerified: result.freedom_target_verified === true,
    observedWebUrl: typeof result.observed_web_url === "string" ? result.observed_web_url : null,
    observedFreedomUrl: typeof result.observed_freedom_url === "string" ? result.observed_freedom_url : null,
    observedTxtValues: Array.isArray(result.observed_txt_values)
      ? result.observed_txt_values.filter((value): value is string => typeof value === "string")
      : [],
    records: result.records && typeof result.records === "object" ? result.records : {},
    acceptedAnchorHeight: typeof result.accepted_anchor_height === "number" ? result.accepted_anchor_height : null,
    acceptedAnchorBlockHash:
      typeof result.accepted_anchor_block_hash === "string" ? result.accepted_anchor_block_hash : null,
    acceptedAnchorRootHash:
      typeof result.accepted_anchor_root_hash === "string" ? result.accepted_anchor_root_hash : null,
    proofRootHash: typeof result.proof_root_hash === "string" ? result.proof_root_hash : null,
    observationProvider: typeof result.observation_provider === "string" ? result.observation_provider : null,
    failureReason: typeof result.failure_reason === "string" ? result.failure_reason : null,
  }
...
export async function mintSpacesChallenge(
  ...
  const rootLabel = normalizeRootLabel(normalizedRootLabel)
  const routeUrl = `https://${domain}/c/@${rootLabel}`
