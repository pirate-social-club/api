...
function ladder(count: number, options: Partial<Parameters<typeof scored>[0]> = {}): ScoredVideoCandidate[] {
  return Array.from({ length: count }, (_, index) => scored({
...
describe("takeVideoFeedPage", () => {
  ...
  test("reserves slots for under-measured items over higher-scoring measured ones", () => {
    const measured = Array.from({ length: 20 }, (_, index) => scored({
      postId: `pst_hit_${index}`,
      score: 10 - index / 100,
      validImpressions: 5_000,
    }))
    const fresh = Array.from({ length: 8 }, (_, index) => scored({
  ...
  test("returns unfilled reserved slots to the general pool", () => {
    const measured = Array.from({ length: 10 }, (_, index) => scored({
      postId: `pst_hit_${index}`,
      score: 10 - index,
      validImpressions: 5_000,
    }))
    const page = takeVideoFeedPage([...measured, scored({
      postId: "pst_new",
      score: 0.01,
      validImpressions: 0,
    })], 6)
