...
export async function recoverRewardCampaignIncident(input: {
  ...
  const kind = requiredString(incident, "incident_kind")
  if (kind !== "accounting_mismatch") {
    ...
    if (!rpcUrl || !Number.isSafeInteger(chainId)) throw conflictError("Campaign finality verification is unavailable")
    ...
    for (const effect of effects.rows) {
      ...
      if (!txHash || !Number.isSafeInteger(blockNumber) || !blockHash) throw conflictError("Campaign funding provenance is incomplete")
      const [receipt, block] = await Promise.all([provider.getTransactionReceipt(txHash), provider.getBlock(blockNumber)])
      if (!receipt || receipt.blockNumber !== blockNumber || receipt.blockHash.toLowerCase() !== blockHash || block?.hash?.toLowerCase() !== blockHash) {
        throw conflictError("Campaign funding finality is not healthy")
      }
    }
  }
  ...
  return withTransaction(input.client, "write", async (tx) => {
    ...
    const resolved = await tx.execute({
      sql: `UPDATE reward_campaign_incidents SET resolved_at = ?4, resolved_by = ?5, resolution_note = ?6, incident_version = incident_version + 1 WHERE reward_campaign_incident_id = ?1 AND reward_campaign_id = ?2 AND incident_version = ?3 AND resolved_at IS NULL RETURNING reward_campaign_incident_id`,
      args: [input.incidentId, input.campaignId, input.incidentVersion, now, input.operatorActorId, note],
    })
    if ((resolved.rowsAffected ?? resolved.rows.length) !== 1) throw conflictError("Reward campaign incident changed; reload before recovery")
    const open = await tx.execute({
      sql: `SELECT 1 AS present FROM reward_campaign_incidents WHERE reward_campaign_id = ?1 AND resolved_at IS NULL AND incident_kind IN ('accounting_mismatch', 'funding_finality_failure') LIMIT 1`,
      args: [input.campaignId],
    })
    if (open.rows[0]) throw conflictError("Campaign has other unresolved incidents")
