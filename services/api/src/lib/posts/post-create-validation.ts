...
export function assertPostCreateRequest(body: CreatePostRequest, _communityId: string): void {
  ...
  if (
    ...
    throw badRequestError("publish_mode async is only supported for song posts or file posts")
  }
  ...
  if (body.post_type === "file") {
    const contentLabel = "file"
    const sourceRef = body.file_upload
    if (!body.title?.trim()) {
      throw badRequestError(`title is required for ${contentLabel} posts`)
    }
    if (!sourceRef?.trim()) {
      throw badRequestError(`file_upload is required for ${contentLabel} posts`)
    }
  ...
  if (
    ...
    throw badRequestError("access_mode is only supported for song, video, and file posts")
  }
  if (body.post_type !== "song" && body.post_type !== "video" && body.post_type !== "file") {
