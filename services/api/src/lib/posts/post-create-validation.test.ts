...
function genericRequest(overrides: Partial<CreatePostRequest> = {}): CreatePostRequest {
  return {
...
describe("assertPostCreateRequest generic digital goods", () => {
  test("accepts locked file requests", () => {
    expect(() => assertPostCreateRequest(genericRequest(), COMMUNITY_ID)).not.toThrow()
  })

  test("requires source reference, title, and a license for locked goods", () => {
    expect(() => assertPostCreateRequest(genericRequest({ file_upload: "" }), COMMUNITY_ID)).toThrow(/file_upload is required/)
    expect(() => assertPostCreateRequest(genericRequest({ title: "" }), COMMUNITY_ID)).toThrow(/title is required/)
    expect(() => assertPostCreateRequest(genericRequest({ license_preset: null }), COMMUNITY_ID)).toThrow(/license_preset is required/)
  })
  ...
  test("rejects royalty and legacy media fields", () => {
    expect(() => assertPostCreateRequest(genericRequest({
