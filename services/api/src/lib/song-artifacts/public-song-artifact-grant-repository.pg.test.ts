...
const PG_ENV = {
  CONTROL_PLANE_DATABASE_URL: `postgres://artifact-grant@localhost:5432/${TEST_DB}`,
} as unknown as Env
...
describe.skipIf(!RUN)("public song artifact grants (real Postgres)", () => {
  ...
  beforeAll(async () => {
    ...
    await database.unsafe(`
      ...
      ) VALUES ('cmt_music', 'pst_video', 'published', 'public', '{"access_mode":"public"}'::jsonb);
  ...
  test("executes the read-time authorization query in PostgreSQL and revokes stale grants", async () => {
    setControlPlanePostgresPoolFactoryForTests(() => {
      const query = async (sql: string, values?: unknown[]) => ({
        rowCount: null,
        rows: await database.unsafe(sql, values ?? []) as Record<string, unknown>[],
      })
      return {
