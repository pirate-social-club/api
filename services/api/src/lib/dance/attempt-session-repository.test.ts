...
function fakeClient(budget = { recent_count: 0, active_count: 0, idempotency_count: 0 }) {
  ...
  const revision = {
    ...
    scorer_version: "dance_scorer_gate0_v2",
  ...
  const execute = async (query: string | { sql: string; args?: unknown[] }) => {
    ...
    if (sql.includes("INSERT INTO dance_attempt_sessions")) {
      ...
      session = {
        ...
        start_cue_policy_version: args[29],
        start_cue_kind: args[30],
        start_cue_minimum_hold_ms: args[31],
        start_cue_observation_window_ms: args[32],
        upload_object_key: args[33],
        maximum_bytes: args[34],
