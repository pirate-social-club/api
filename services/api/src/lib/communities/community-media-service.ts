...
function assertSupportedMimeType(kind: CommunityMediaKind, mimeType: string): void {
  if (!allowedMimeTypes.has(mimeType)) {
    throw badRequestError(`${kind} must be a JPEG, PNG, WebP, or GIF image`)
  }
}

function assertFileSize(kind: CommunityMediaKind, sizeBytes: number): void {
...
function resolveObjectKey(kind: CommunityMediaKind, mimeType: string): { objectKey: string; objectName: string } {
  const objectName = `${makeId(kind)}.${extensionForMimeType(mimeType)}`
  return {
    objectKey: `community-media/${kind}/${objectName}`,
    objectName,
  }
}
...
function buildMediaRef(origin: string, kind: CommunityMediaKind, objectName: string): string {
  return new URL(`/community-media/${kind}/${objectName}`, origin).toString()
}
...
export function assertCommunityMediaObject(input: {
  ...
  const kind = input.kind === "avatar" || input.kind === "banner"
    ? input.kind
    : null
  if (!kind) {
    throw notFoundError("Community media not found")
  }
