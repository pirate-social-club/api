...
import { reconcileStuckPostPublishFinalizeJobs } from "./post-publish-finalize-handler"
...
describe("reconcileStuckPostPublishFinalizeJobs prelude deadline", () => {
  ...
  it("starts no community when the deadline is already spent", async () => {
    ...
    expect(summary.failed_posts).toBe(0)
  })
  ...
  it("rotates the scan order so consecutive truncated ticks cover different communities", async () => {
    ...
    const runTick = (minuteMs: number) => {
      let clock = minuteMs
      return reconcileStuckPostPublishFinalizeJobs({
        env,
