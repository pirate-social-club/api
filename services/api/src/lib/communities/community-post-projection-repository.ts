...
function isMissingProjectionMetricsColumnError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error)
  return message.includes("community_post_projections") && message.includes("upvote_count")
}
...
export async function recordCommunityPostProjection(
  ...
  try {
    await tx.batch([
      ...
      if (!isMissingProjectionMetricsColumnError(error)) {
        throw error
      }
