...
describe("community membership gate routes", () => {
  ...
  test("Very-verified members can vote in Very OR proof-of-work communities without ALTCHA", async () => {
    ...
    expect(previewBody.membership_gate_expression).toEqual({
      op: "or",
      children: [
        { op: "gate", gate: { gate_id: expect.stringMatching(/^gate_content_[a-f0-9]{32}$/), gate_type: "unique_human", accepted_providers: ["very"] } },
        { op: "gate", gate: { gate_id: expect.stringMatching(/^gate_content_[a-f0-9]{32}$/), gate_type: "altcha_pow" } },
      ],
    })
