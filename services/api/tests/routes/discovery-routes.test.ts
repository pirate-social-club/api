...
describe("discovery routes", () => {
  ...
  test("GET /.well-known/mcp/server-card.json advertises the API catalog resource", async () => {
    ...
    expect(body.tools.map((tool) => tool.name)).toContain("search_board")
    expect(body.tools.map((tool) => tool.name)).toContain("get_thread")
    expect(body.tools.map((tool) => tool.name)).toContain("get_my_activity")
  ...
  test("POST /mcp lists community write tools", async () => {
    ...
    expect(body.result.tools.map((tool) => tool.name)).toContain("search_board")
    expect(body.result.tools.map((tool) => tool.name)).toContain("get_thread")
    expect(body.result.tools.map((tool) => tool.name)).toContain("get_my_activity")
