...
describe("community media routes", () => {
  ...
  test("rejects unsupported media types before any bucket write", async () => {
    ...
    expect(body.message).toBe("banner must be a JPEG, PNG, WebP, or GIF image")
    expect(filebaseCalled).toBe(false)
  })

  test("reuses FILEBASE_S3_BUCKET_MUSIC when the shared media bucket is absent", async () => {
    const storedObjects = new Map<string, { body: Uint8Array; contentType: string }>()
    globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      const request = input instanceof Request ? input : new Request(input, init)
      if (!request.url.startsWith("https://s3.filebase.test/")) {
    ...
    const ctx = await createRouteTestContext({
      ...
      FILEBASE_S3_SECRET_KEY: "test-filebase-secret",
      FILEBASE_S3_ENDPOINT: "https://s3.filebase.test",
      FILEBASE_S3_BUCKET_MUSIC: "legacy-music-bucket",
    })
    cleanup = ctx.cleanup

    const session = await exchangeJwt(ctx.env, "community-media-legacy-bucket")
    const fileBytes = new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10, 5, 6, 7, 8])
    const formData = new FormData()
    formData.set("kind", "avatar")
    formData.set("file", new File([fileBytes], "avatar.png", { type: "image/png" }))
    ...
    const payload = await json(upload) as { storage_bucket: string }
    expect(payload.storage_bucket).toBe("legacy-music-bucket")
  })
})
